...
/**
 * package/file: cc.openhome.model/UserService
 * author: pcwuyu
 * version: 2016/9/19 16:01
 */
public class UserService {
    ...
    public boolean isInvalidUsername(String username) {  //是否为不合法用户名称
    ...
    public boolean checkLogin(String username, String password) throws IOException {
        if (username != null && password != null) {
            for (String file : new File(USERS).list()) {
                if (file.equals(username)) {
                    BufferedReader reader = new BufferedReader(new FileReader(USERS + "/" + "/profile"));
    ...
    public Map<Date, String> readMeaasge(String username) throws IOException {  //读取用户的信息
    ...
    public void addMessage(String username, String blabala) throws IOException {  //新增用户信息
        String file = USERS + "/" + username + "/" + new Date().getTime() + ".txt";
        BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), "UTF-8"));
        writer.write(blabala);
        writer.close();
    }

    public void deleteMessage(String username, String message) {  //删除用户信息
